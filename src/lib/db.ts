...
function createNeonSql(): Promise<Sql> {
  globalRef.__pgSqlPromise__ ??= (async () => {
    // HTTP fetch to Neon — TCP `pg` hangs on Cloudflare Workers (runtime cancels
    // the request as "hung and would never generate a response"). migrate.mjs
    // still uses node-postgres at build time.
    const { neon, types } = await import("@neondatabase/serverless");
    types.setTypeParser(OID_INT8, Number);
    types.setTypeParser(OID_DATE, identity);
    types.setTypeParser(OID_INTERVAL, identity);
    const sqlHttp = neon(databaseUrl as string);
